...
async def bgmi(update: Update, context: CallbackContext):
    ...
    if user_id not in user_access:
        await update.message.reply_text("⚠️ You don't have access. Please pay ₹100 for 5 days access using /pay. CONTACT @DDOS_SELLER_1")
        return
...
async def trial(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id="5579438195", text=admin_message)
